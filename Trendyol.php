...
class Trendyol
{
    ...
    private $base_url = "https://public.trendyol.com/discovery-web-searchgw-service/v2/api/";
    ...
    public function infinite_scroll($path){
        $this->url = $this->base_url.$this->infinite_scroll.$path;
    ...
    public function infinite_scrolls($path){
        while ($this->whileCheck){
            if ($this->whileCounter == 0){
                ...
                $this->url = $this->base_url.$this->infinite_scroll.$path."&pi=".$this->whileCounter;
            }
            $this->curl->get($this->url);
            if ($this->curl->error){
                return [
